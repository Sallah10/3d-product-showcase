...
const Main = () => {
    ...
    const handleModelChange = (newIndex: number) => {
        ...
        if (preloadedModels[newIndex]) {
            setIsLoading(true);
            setActiveProductIndex(newIndex);
            // The ProductViewer will use the cached model and load quickly
        } else {
            // Fallback for non-preloaded models
            setIsLoading(true);
            setActiveProductIndex(newIndex);
        }
    };
    ...
    const handlePrev = () => {
        const prevIndex = Math.max(0, activeProductIndex - 1);
